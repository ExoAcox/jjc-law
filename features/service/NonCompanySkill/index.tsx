    ...
    return <Responsive parentClassName="border-y">
        <div className="flex flex-col">
            <div>
                ...
                <p className="mt-2 mb-16 text-lg max-w-[600px]">{dict.service.non_company_skill.desc}</p>
            </div>
            <div className="grid grid-cols-2 gap-12">
                {dict.service.non_company_skill.contents.map((content, index) => {
                    return <div key={index} className="py-4 px-6 bg-amber-100 rounded-lg flex-1 text-gray-700 dark:bg-amber-200">
                        <h5 className="border-b border-stone-400 pb-2.5 mb-4 ">{content.title}</h5>
                        <p>{content.desc}</p>
                    </div>
                })}
            </div>
